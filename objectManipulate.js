		...
		this.queries.draggable.results.forEach((entity) => {
			...
			switch (draggable.state) {
				...
				case "to-be-detached":
					...
					break;
				default:
					console.log("dragging", object.scale);
					break;
			}
		});
	}
...
class Scaleable extends Component {}

Scaleable.schema = {
	// scale state: [none, scaling]
	state: { type: Types.String, default: "none" },
	startDistance: { type: Types.Number, default: 0 },
	startScale: { type: Types.Ref, default: new THREE.Vector3(1, 1, 1) },
	attachedPointers: { type: Types.Array },
};

class ScaleSystem extends System {
	execute(/*delta, time*/) {
		this.queries.scaleable.results.forEach((entity) => {
			const scaleable = entity.getMutableComponent(Scaleable);
			console.log("146 scaleable", scaleable);
			const object = entity.getComponent(Object3D).object;

			if (
				scaleable.state == "scaling" &&
				scaleable.attachedPointers.length == 2
			) {
				console.log("continue to scale");
				const currentDistance =
					scaleable.attachedPointers[1].controller.position.distanceTo(
						scaleable.attachedPointers[0].controller.position
					);
				console.log("currentdistance", currentDistance);
				console.log("scaleabledistance", scaleable.startDistance);
				const scaleFactor = currentDistance / scaleable.startDistance;
				// const scaleFactor = currentDistance;
				const newScale = scaleable.startScale
					.clone()
	...
	execute(/*delta, time*/) {
		this.handPointers.forEach((hp) => {
			...
			if (distance) {
				...
				if (intersectingEntity.hasComponent(Scaleable)) {
					const scaleable = intersectingEntity.getMutableComponent(Scaleable);
					const object = intersectingEntity.getComponent(Object3D).object;
					if (hp.isPinched()) {
						if (
							scaleable.state == "none" &&
							scaleable.attachedPointers.length == 0
						) {
							scaleable.state = "single-scaling";
							scaleable.attachedPointers.push(hp);
						}

						if (
							scaleable.state == "single-scaling" &&
							scaleable.attachedPointers.length == 1 &&
							scaleable.attachedPointers[0] != hp
						) {
							scaleable.state = "scaling";
							scaleable.attachedPointers.push(hp);
							scaleable.startScale.copy(object.scale);
							scaleable.startDistance =
								scaleable.attachedPointers[1].controller.position.distanceTo(
									scaleable.attachedPointers[0].controller.position
								);
							console.log("269", scaleable.startDistance);
						}
					} else {
						if (scaleable.state == "scaling") {
							scaleable.state = "single-scaling";
							scaleable.attachedPointers.pop(hp);
							scaleable.startDistance = 0; //set to default value
						}
...
function init() {
	...
	for (let i = 0; i < 1; i++) {
		...
		entity.addComponent(Scaleable); // Adding Scaleable component
	}
